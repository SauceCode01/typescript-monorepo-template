  ...
  return (
    <div className="flex flex-col gap-2 items-start w-full border p-4 rounded-md overflow-auto">
      ...
      {isLoading ? (
        ...
        <>
          {data?.data?.map((studyJam) => (
            <StudyJamCard key={studyJam.id} studyJam={studyJam} />
          ))}
        </>
      )}
    </div>
